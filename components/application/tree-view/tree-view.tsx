...
const Connector = ({ type, size }: { type: "line" | "end" | "none"; size: "sm" | "md" }) => (
    <div className="relative size-4 shrink-0" data-tree-connector>
        {type === "line" && (
            <div
                className={cx("absolute top-1/2 left-1/2 w-px -translate-x-1/2 -translate-y-1/2 bg-[var(--ui-border-secondary)]", size === "sm" ? "h-[42px]" : "h-[44px]")}
            />
        )}
        {type === "end" && (
    ...
    return (
        <TreeViewContext.Provider value={contextValue}>
            <AriaTree
                ...
                className={(state) => cx("font-barlow flex flex-col", typeof className === "function" ? className(state) : className)}
            />
        ...
        <AriaTreeItemContent>
            {(state) => {
                ...
                if (showConnectors && itemKey != null && level > 1) {
                    ...
                    /*
                     * Items with children reserve a slot via their own chevron button below.
                     * Leaf items render no chevron, so without this spacer they'd end up with
                     * one fewer slot than a sibling with children — landing flush with their
                     * parent instead of indented one level further.
                     */
                    if (!hasChildItems) {
                        connectors.push(<Connector key="self" size={size} type="none" />);
                    }
                }
